"""
Command-Line Interface for CardioMRI-Strain: Feature Tracking & Global Longitudinal Strain Agent.
"""
import argparse
import csv
import json
...
from .agents import CardioMRICoordinator

coordinator = CardioMRICoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="cardiac-mri-cine-strain-agent", description="CardioMRI-Strain: Feature Tracking & Global Longitudinal Strain Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  CARDIOMRI-STRAIN: FEATURE TRACKING & GLOBAL LONGITUDINAL STRAIN AGENT")
        print(f"  Domain: Cardiology AI | Standard: SCMR Recommended CMR Protocols 2020")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting CardioMRI-Strain: Feature Tracking & Global Longitudinal Strain Agent on http://{args.host}:{args.port}")
